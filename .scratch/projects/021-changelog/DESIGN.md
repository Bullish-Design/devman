...
## Revision 3 — jj change ids, and pyjutsu instead of the `jj` CLI

Landed after real use surfaced two things revision 2 didn't get right.

**Identity: a jj change id, not a git commit hash.** `land`'s own rebase gives
a folded-in commit a new git hash every time; a change id survives every
rewrite gitman makes. Both the entry filenames and both receipts
(`.changelog-entries.last`, `CHANGELOG.md`'s own marker) are change ids now.
Each `CHANGELOG.md` bullet is an Obsidian wikilink, `[[<change id>|<subject>]]`
— chosen over a markdown link because `.devman/changelog/entries/` is
gitignored (§5.3 below), so a markdown link from the committed file would be
dead for anyone without that machine's local cache; a wikilink degrades to
plain bracketed text everywhere else and costs nothing.

**Neither workflow shells out to the `jj` CLI.** Reading gitman's own
`pyproject.toml` before shipping found the assumption wrong: gitman binds
`pyjutsu`, jj-lib's in-process Python binding, and states outright *"there is
no `jj` CLI dependency."* `gitman doctor` checks for `pyjutsu` and `git`
(jj's escape hatch), never `jj`. Both workflows now call `python3`, importing
`pyjutsu.Workspace` for the same revset ranges (`::trunk & ~root()`,
`last..trunk`, verified to take identical strings to the CLI) and rendering
the diff detail themselves, since pyjutsu's `Diff`/`DiffStat` are structured
data (hunks with no surrounding context) rather than a byte-exact patch.

Both changes were validated the same way as revision 2's measurements — in
scratch jj repos, running the exact shipped script content extracted from the
YAML, not a hand-typed approximation of it.

**`.devman/changelog/entries/` is gitignored, not committed** — added to
`.git/info/exclude` (never a tracked `.gitignore`, matching how this
repository already excludes `.devman/.runs/` and `.devman/gitman/`), on the
same reasoning as `run.py`'s and CONCEPT.md §9.2's: it is regenerable local
cache, not a durable record on its own. The durable record is `CHANGELOG.md`,
which is why its bullets are the wikilinks and not the other way around.

## Revision 4 — daemon environment boundaries and a self-contained model call

The first real adopter run found two deployment assumptions that revision 3 did
not measure. Dagu runs a workflow step with its daemon environment, not the
interactive devenv environment. Therefore direct `python3` could not import
`pyjutsu`, and direct `gitman` could not find the adopter's gitman executable.
The workflows now resolve both through `devenv shell`, while keeping the
project's own Python and gitman installation authoritative.

The same run found that the group did not ship `scripts/gpu_complete.py`, and
neither intended taker had that file. The summary step now uses a short
standard-library OpenAI-compatible client in a temporary file. It keeps the
same `$GPU_LLM_BASE_URL` and `$GPU_LLM_MODEL` contract, the 60,000-character
input bound, and the `gpu` queue. This keeps the group self-contained and
leaves the local model dependency explicit.

## Revision 5 — console scripts, and what that leaves unfinished (023 phase 2d)

Revision 3 chose `pyjutsu` over the `jj` CLI on a correct premise and a wrong
conclusion. The premise holds: gitman binds pyjutsu in process, `gitman doctor`
never checks for `jj`, and this repository's own devenv has no `jj` binary. The
conclusion — that a step may therefore *import* pyjutsu — does not.

**A console script is a property of the machine; a library import is a property
of the interpreter that imports it.** The shared toolchain venv lends `gitman`
and `templateer` through PATH, and PATH cannot lend a library through
`sys.path`. So `import pyjutsu` failed in every adopter's devenv, including this
one. Revision 4 read the same failure and drew a third wrong conclusion from it
— that `devenv shell` was the missing piece. It is necessary and not sufficient:
`devenv shell -- python3 -c 'import pyjutsu'` still fails, while
`devenv shell -- gitman` works.

The expensive part was never the failure, which is loud. It is that
`repoman doctor` and `gitman doctor` both report pyjutsu green while the step
fails, because each asks its own interpreter and neither asks the step's
(023-toolchain P1). `changelog.yaml`'s gate now probes for `gitman` and
`templateer` as its first action, inside the same `devenv shell --` every later
command uses — the only PATH whose answer is relevant.

**`changelog.yaml` is converted; `changelog-entries.yaml` is not, and cannot
be.** `gitman log --revset <a>..<b> --json` (023 phase 2a) supplies the commit
range the summary workflow needs. The entries workflow needs diffs, and gitman
has no diff verb. Closing it takes a new `gitman diff --json`, or reversing this
revision's own first paragraph. Until then the post-land chain does not run and
`changelog` is a by-hand verb. This is stated in the group README rather than
left for the next reader to rediscover.

Revision 4's standard-library model client is also gone. `templateer` renders
the summary through the group's own template, called by name off PATH — not
through an interpreter search-path variable and `uv run --project` at an
absolute sibling checkout, which is a directory layout that exists on one
machine (023-toolchain P2).

**The finish check is `failure_reason`, not `finish_reason`.** This endpoint
returns `finish_reason: "length"` with `content: ""` when the model spends its
budget on reasoning content — a successful HTTP 200 carrying nothing
(023-toolchain P6). `templateer --json` does not forward `finish_reason`; it
emits `GenerationResult.model_dump()`, whose own validator forbids a success
that carries no artifact. Truncation therefore surfaces as `failure_reason`, and
that is what the step checks. A separate check refuses a whitespace-only
artifact, covering the one case the first misses: the schema permits
`summary: ""`, which would otherwise append an empty section, advance the
frontier past those commits, and exit 0.

**`TEMPLATEER_TEMPLATE_PATH` is repository-relative.** It held one machine's
absolute checkout path, which `groups/README.md` forbids a group workflow from
carrying. It resolves for devman, which owns the templates and adopts its own
groups, and the gate refuses loudly for anyone else. Shipping a group's assets
to an adopter is 022's open question 2 and this revision does not close it:
`modules/devenv.nix` reads a workflow with `builtins.readFile` and projects the
text, and there is no asset path beside it.

Measured on the live endpoint: `templateer --model gemma` returns
`failure_reason: config_error`, `Unknown model: gemma`. templateer is a
pydantic-ai caller and names a model `<provider>:<id>`, so the workflow sends
`openai:$GPU_LLM_MODEL` and `GPU_LLM_MODEL` stays the one id `/v1/models` lists.
