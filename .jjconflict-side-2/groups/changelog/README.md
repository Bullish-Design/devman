...
No task name to define — both workflows are plain shell (with an inline
`python3` step where the logic needs `pyjutsu`), no devenv task needed, unlike
`release`'s `release:build`. It costs five things instead, each a one-time
setup:
...
cid = pyjutsu.Workspace.load('.').resolve(trunk).change_id
print(f'<!-- devman-changelog: covers up to {cid} -->')
" > CHANGELOG.md
...
**4. `pyjutsu` importable from `python3`.** Both workflows read jj's history
through `pyjutsu.Workspace`, the same in-process library gitman itself is
built on — never the separate `jj` CLI binary (see "Why pyjutsu, never the
`jj` CLI" below). Wherever `gitman` runs, `pyjutsu` is already a dependency;
this only matters if the workflow's `python3` resolves to a *different*
environment than the one gitman was installed into.

**5. A reachable `$GPU_LLM_BASE_URL`**, exactly as `.devman/workflows/
...
## Why pyjutsu, never the `jj` CLI

The first draft of both workflows shelled out to `jj log`/`jj show`. Reading
gitman's own `pyproject.toml` before shipping it found the assumption wrong:
*"pyjutsu >= 0.20 binds the complete in-process surface ... there is no `jj`
CLI dependency."* `gitman doctor` itself checks for `pyjutsu` and `git` (for
jj's "escape hatch"), never `jj`. A repository can run gitman with no `jj`
binary on PATH at all, so a workflow that shells out to one would fail in
exactly that repository — the one this group is for.

Both workflows instead call `python3`, importing `pyjutsu.Workspace` directly:
`.log(revset)` for the commit range, `.diff`/`.diff_stat` for the detail file,
`.resolve(change_id)` for a single commit's description. The revset language
is unchanged (`::trunk & ~root()`, `last..trunk`) — pyjutsu takes the same
strings jj's CLI does, verified in a scratch repo before either workflow was
written. Its diff is structured (hunks with no surrounding context, by
pyjutsu's own design), so the entry file's rendering is this group's own, not
a copy of `git diff`'s output — a deliberate trade, since the file is read by
a person or an LLM, never replayed as a patch.
