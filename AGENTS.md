...
- an unattended write to **trunk** — every other write is tiered (015): a new
  file and agent surface are `free`; an edit to existing tracked source lands on
  a **gitman lane** for a person to merge; `insitu` is `format`'s bounded
  exception and nothing else holds it
