...
# THE IDENTITY GRAMMAR IS NO LONGER DEFINED HERE (038 Stage 16). It moved to
# `devman_contract.identity`, unchanged, so that the independent `devman_link`
# adapter can validate a name without importing the registry. This module
# re-exports all three names, so every existing `from .registry import
# identity_fault` keeps working.
from devman_contract.identity import (  # noqa: F401
    IDENTITY_GRAMMAR,
    IDENTITY_PATTERN,
    identity_fault,
)
...
def report(exc: Exception) -> None:
    ...
    wording is the refusal, not decoration. It takes any exception because
    `cli.main` prints a `devman_link` refusal this way as well (038 Stage 16),
    and that component cannot import `RegistryError` without importing the
    plane it is independent of. `cli.main` prints one this way when
