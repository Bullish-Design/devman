...
four things instead, each a one-time setup:
...
output is per working tree). It is regenerable from `git log` at any time, so
...
**4. A reachable `$GPU_LLM_BASE_URL`**, exactly as `.devman/workflows/
