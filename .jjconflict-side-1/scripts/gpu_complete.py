...
Points at any OpenAI-compatible server named by `GPU_LLM_BASE_URL`. devman
...
def main() -> int:
    ...
    if not model_name:
        print(
            "gpu_complete: GPU_LLM_MODEL is unset — name the model the server loaded",
            file=sys.stderr,
        )
        return 2
    ...
    # api_key is required by the OpenAI client shape and unchecked by a local
    ...
    try:
        ...
        print(
            f"gpu_complete: {base_url} did not answer — is the server running? ({exc})",
            file=sys.stderr,
        )
        return 1
